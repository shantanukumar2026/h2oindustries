...
const US_CITIES = [
  ...
  { city: "Houston, TX", annualRain: 53, description: "Gulf Coast Subtropical - High intensity convective convective storms" },
...
// CN values are standard civil engineering reference numbers in the US
const LAND_USES = [
  { 
    id: "paved", 
...
export default function CalculatorPage() {
  ...
  // 1. Potential Maximum Retention (S) in inches:
  // S = (1000 / CN) - 10
  const S = (1000 / curveNumber) - 10;

  // 2. Initial Abstraction (Ia) in inches (amount of rain captured before runoff starts, standard is 0.2*S)
  const Ia = 0.2 * S;

  // 3. Runoff Depth (Q) in inches:
  ...
  // 4. Runoff Volume in Cubic Feet = Area (sqft) * (Runoff Depth (inches) / 12)
  const runoffCubicFeet = areaInSqFt * (runoffDepth / 12);
  const runoffGallons = runoffCubicFeet * 7.48052;

  // ──── WATER QUALITY VOLUME (WQV) ────
  // WQV = Runoff from 1.0 inch design storm (often used for EPA compliance)
  let wqvDepth = 0;
  if (1.0 > Ia) {
  ...
  // ──── ANNUAL POLLUTANT TSS CAPTURE ────
  // Annual Runoff Volume based on actual city annual rainfall
  ...
  // H2 Industries filters achieve 85% TSS removal efficiency
  // Standard US urban runoff TSS concentration is 150 mg/L
  // annual TSS lbs = (Annual Gallons * 3.785 L/Gal * 150 mg/L * 85% removal) / 453,592 mg/lb
  const annualTSSRemovedLbs = (annualRunoffGallons * 3.78541 * 150 * 0.85) / 453592;

  // ──── US REGULATORY COMPLIANCE STATUS ────
  // - EISA Section 438 requires matching pre-development hydrology (95th percentile, typically ~1.2 inches)
  // - NPDES MS4 permit requires retaining WQV on-site.
      ...
      <div style={{ paddingTop: 140, paddingBottom: 80, flex: 1 }}>
        <div style={{ maxWidth: 1440, margin: "0 auto", padding: "0 40px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1.2fr 0.8fr", gap: 48 }} className="calc-layout">
            ...
            {/* Left Column: Form Inputs & Formula Display */}
            ...
            {/* Right Column: Output Reports & EPA Rules */}
            <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
